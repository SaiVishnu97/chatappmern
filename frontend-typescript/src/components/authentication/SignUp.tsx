...
import { SignUpInitialValues } from './AuthenticationTypesDeclarations';
import { useAppDispatch,addNewProperties } from 'state/index';
...
const SignUp = () => {
    ...
    const onSubmit= async(values:SignUpInitialValues) => {
      try{
      setLoading(true);
    ...
    const imageUpload=(event: React.ChangeEvent<HTMLInputElement>)=>
    {
        ...
        if(target.files&&target.files[0]){
          if(imgref.current){
            imgref.current.src=URL.createObjectURL(target.files[0]);
