...
import { addNewChats, useAppDispatch } from 'state'
...
const GroupChatModal:React.FC<GroupChatModalProps> = ({ isOpen, onClose }) => {
  ...
  React.useEffect(()=>{
    const newsetusers=actualuserslist.filter((val:User)=>!selectedusers.some((val1:User) => val1.email === val.email));
    ...
    <div className='GroupChatModal'>
        ...
        <ModalBody>
            ...
            {loading?<Spinner thickness='4px' size={'xl'}/>:userslist.map((val)=>{
              if(val._id===currentuserdetails._id)
                return (<></>)
