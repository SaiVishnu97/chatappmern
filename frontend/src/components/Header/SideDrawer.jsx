...
const SideDrawer = ({isOpen,onClose}) => {
  ...
  const selectChat=async(userdetails)=>{
    ...
    try
    {
      ...
      onClose();
      const results=await axios.post(process.env.REACT_APP_BACKENDURL+'/api/chats',{userId:userdetails._id},config);
  ...
  return (
    <div>
        ...
        <DrawerContent>
          ...
          <Button onClick={getUsers} onMouseOver={(event)=>
event.target.style.background = '#a19d9d'}
onMouseOut={(event)=>event.target.style.background='#E8E8E8'} style={{width:'max-content',color:'black',padding:'4%',fontWeight:'700' ,backgroundColor:'#E8E8E8'}} >
            Go
          </Button>
          </div>
          <DrawerBody>
